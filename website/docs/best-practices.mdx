...
## Bail out before touching props

Middlewares run for **every JSX element, on every render**. The middleware chain itself is very cheap, but cloning `props` is not free — and it is easy to clone them accidentally for every element in your app by destructuring in the function signature:

```jsx
// ⚠️ This clones props for EVERY element, even ones without $ripple
function rippleMiddleware(next, type, { $ripple, ...props }, key) {
  // ...
}
```

Instead, check whether your middleware applies *before* creating any new objects, and pass the arguments through untouched when it doesn't:

```jsx
function rippleMiddleware(next, type, props, key) {
  // Elements without the directive pass through with zero allocations
...
In a benchmark with three middlewares, the bail-early style reduced element creation overhead by about 60% compared to destructuring in the signature. The same applies to spreading props (`{ ...props, something }`) — only do it on the elements your middleware actually changes.
